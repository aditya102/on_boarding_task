...
class AccountsTestCase(TestCase):
    ...
    def test_login_page(self):
        ...
        self.assertContains(
            response, '<div id="div_id_username" class="form-group"> <label for="id_username" class="col-form-label  requiredField">')
        self.assertContains(
            response, '<span class="asteriskField">*</span> </label> <div class=""> <input type="password" name="password" class="textinput textInput form-control" required id="id_password" /> </div> </div')
        self.assertContains(
            response, '<button class="btn btn-outline-info" type="submit">Login</button>')

    def test_post_login_page(self):
        response = self.client.post(reverse(
            'login'), {'username': 'demo_user', 'password': 'demo@123'}, follow=True)
